...
export function VocabularyView() {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-4 rounded-3xl bg-white dark:bg-[#141414] border border-[#CBD5E1] dark:border-white/10 shadow-xs">
        {/* Search */}
        <div className="relative w-full md:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 text-base sm:text-xs rounded-xl bg-[#F8FAFC] dark:bg-[#1C1C1C] border border-[#CBD5E1] dark:border-white/10 text-[#0F172A] dark:text-[#FFFFFF] placeholder:text-[#475569] outline-hidden focus:border-[#00638E]"
          />
        </div>
            ...
            <button
              ...
              className={clsx(
                ...
                selectedCategory === cat
                  ...
                  : 'bg-[#F1F5F9] dark:bg-[#1C1C1C] border border-[#CBD5E1] dark:border-white/10 text-[#475569] dark:text-[#7A8992] hover:text-[#0F172A] dark:hover:text-white hover:bg-[#E2E8F0] dark:hover:bg-[#2B2B2B]'
              )}
            >
            ...
            >
              <div
                className={clsx(
                  ...
                  isMastered
                    ? 'bg-[#F1F5F9] dark:bg-[#1C1C1C] border-2 border-[#00638E]'
                    : 'bg-white dark:bg-[#141414] border-[#CBD5E1] dark:border-white/10'
                )}
              >
                {!isFlipped ? (
                  // Front Face
                  <div className="space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs px-2.5 py-0.5 rounded-full bg-[#F1F5F9] dark:bg-[#1C1C1C] border border-[#CBD5E1] dark:border-white/10 text-[#0F172A] dark:text-[#FFFFFF] font-bold">
                        {card.level}
                      </span>
                      <button
                        onClick={(e) => {
                        ...
                        className="p-2 rounded-xl bg-[#F1F5F9] dark:bg-[#1C1C1C] hover:bg-[#E2E8F0] dark:hover:bg-[#2B2B2B] border border-[#CBD5E1] dark:border-white/10 text-[#00638E] transition-colors tactile-btn"
                    ...
                    <div className="space-y-1 text-center py-6">
                      <h3 className="text-3xl font-serif font-bold text-[#0F172A] dark:text-[#FFFFFF] group-hover:text-[#00638E] dark:group-hover:text-[#8CB9CC] transition-colors">
                        {card.word}
                      </h3>
                      <div className="font-mono text-xs text-[#475569] dark:text-[#7A8992] flex items-center justify-center gap-2">
                        <span>{card.partOfSpeech}</span>
                        <span>·</span>
                        <span className="text-[#00638E] dark:text-[#8CB9CC] font-semibold">{card.ipa}</span>
                      </div>
                    </div>

                    <p className="text-xs text-center text-[#475569] dark:text-[#7A8992] font-mono flex items-center justify-center gap-1">
                  ...
                  <div className="space-y-4 animate-in fade-in duration-200">
                    ...
                    <div className="space-y-2 text-xs">
                      <div>
                        <strong className="text-[#0F172A] dark:text-[#FFFFFF] block">Definisi (ID):</strong>
                        <p className="text-[#1E293B] dark:text-[#BFD8E3] leading-relaxed">{card.definitionId}</p>
                      </div>
                      <div>
                        <strong className="text-[#0F172A] dark:text-[#FFFFFF] block">Definisi (EN):</strong>
                        <p className="text-[#475569] dark:text-[#7A8992] italic leading-relaxed">{card.definitionEn}</p>
                      </div>
                    </div>

                    <div className="p-3 rounded-2xl bg-[#F8FAFC] dark:bg-[#1C1C1C] border border-[#CBD5E1] dark:border-white/10 text-xs text-[#0F172A] dark:text-[#FFFFFF] italic">
                      "{card.exampleSentence}"
                    </div>

                    <div className="pt-2 border-t border-[#CBD5E1] dark:border-white/10">
                      <span className="font-mono text-[10px] text-[#475569] dark:text-[#7A8992] uppercase tracking-wider block mb-1 font-semibold">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {card.collocations.map((c, i) => (
                          <span
                            key={i}
                            className="text-[10px] font-mono bg-[#F1F5F9] dark:bg-[#1C1C1C] px-2 py-0.5 rounded-md border border-[#CBD5E1] dark:border-white/10 text-[#0F172A] dark:text-[#FFFFFF]"
                          >
                            {c}
                          </span>
                        ))}
...
export default function VocabularyPage() {
  ...
  React.useEffect(() => { router.replace('/?tab=vocabulary'); }, [router]);
  return null;
}
