...
export default function MatricesPage() {
  ...
  return (
    <AppShell
      ...
      title="Master Matriks Fondasi Gramatika"
    >
      <div className="space-y-6">
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[#50585C] dark:text-[#7A8992]">Dimensi Waktu:</span>
              ...
              <div className="relative">
                <Search className="w-3.5 h-3.5 absolute left-3 top-2.5 text-[#50585C] dark:text-[#7A8992]" />
                <input
                  type="text"
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Cari rumus atau tenses..."
                  className="pl-8 pr-3 py-1.5 rounded-xl bg-[#EDF3F7] dark:bg-[#1C1C1C] border border-[#BFD8E3]/40 dark:border-white/5 text-xs text-[#141414] dark:text-[#FFFFFF] outline-hidden"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredTenses.map((t) => (
                <div
                  key={t.id}
                  className="p-5 sm:p-6 rounded-3xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 shadow-xs space-y-4 flex flex-col justify-between"
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between pb-2 border-b border-[#BFD8E3]/30 dark:border-white/5">
                      <div className="flex items-center gap-2">
                        <span className="text-xs font-mono font-bold px-2 py-0.5 rounded-md bg-[#00638E]/15 text-[#00638E] dark:text-[#8CB9CC]">
                          {t.timeDimension} • {t.aspect}
                        </span>
                      </div>
                      <button
                        onClick={() => playTextToSpeech(t.academicExample)}
                        className="p-1.5 rounded-lg text-[#50585C] dark:text-[#7A8992] hover:text-[#00638E] cursor-pointer"
                    ...
                    <h3 className="text-lg font-serif font-bold text-[#141414] dark:text-[#FFFFFF]">
                      {t.tenseName}
                    </h3>

                    {/* Formulas */}
                    <div className="p-3 rounded-xl bg-[#EDF3F7] dark:bg-[#1C1C1C] font-mono text-xs space-y-1">
                      <div><span className="text-[#00638E] dark:text-[#8CB9CC] font-bold">(+)</span> {t.positiveFormula}</div>
                      <div><span className="text-rose-500 font-bold">(-)</span> {t.negativeFormula}</div>
                      <div><span className="text-amber-500 font-bold">(?)</span> {t.questionFormula}</div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {t.timeMarkers.map((m, mIdx) => (
                        <span
                          key={mIdx}
                          className="text-[10px] font-mono px-2 py-0.5 rounded-md bg-black/5 dark:bg-white/5 text-[#50585C] dark:text-[#BFD8E3]"
                        >
                          {m}
                        </span>
                      ))}
                    ...
                    {/* Academic Example */}
                    <div className="p-3 rounded-xl bg-[#00638E]/10 border border-[#00638E]/20 text-xs space-y-1">
                      <span className="font-mono text-[10px] uppercase font-bold text-[#00638E] dark:text-[#8CB9CC] block">
                        Contoh Kalimat Akademik:
                      </span>
                      <p className="font-medium text-[#141414] dark:text-[#FFFFFF]">
                        "{t.academicExample}"
                      </p>
                    </div>
                  </div>
        ...
        {activeTab === 'irregular' && (
          <div className="space-y-4">
            <div className="p-4 rounded-2xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 flex flex-wrap items-center justify-between gap-3 text-xs font-mono">
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-[#50585C] dark:text-[#7A8992]">Pola Perubahan:</span>
            ...
            <div className="overflow-x-auto rounded-3xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 shadow-xs">
              <table className="w-full text-left text-xs font-mono">
                <thead>
                  <tr className="border-b border-[#BFD8E3]/40 dark:border-white/5 bg-[#EDF3F7] dark:bg-[#1C1C1C] text-[#50585C] dark:text-[#7A8992]">
                    <th className="p-3.5 sm:p-4">V1 (Base)</th>
                    <th className="p-3.5 sm:p-4">V2 (Past Simple)</th>
                    <th className="p-3.5 sm:p-4">V3 (Past Participle)</th>
                    <th className="p-3.5 sm:p-4">Pola</th>
                    <th className="p-3.5 sm:p-4">Makna Bahasa Indonesia</th>
                    <th className="p-3.5 sm:p-4 text-right">Audio</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#BFD8E3]/20 dark:divide-white/5">
                  {filteredVerbs.map((v) => (
                    <tr key={v.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                      <td className="p-3.5 sm:p-4 font-bold text-[#00638E] dark:text-[#8CB9CC]">
                        {v.v1} <span className="text-[10px] text-[#50585C] dark:text-[#7A8992] font-normal">{v.ipaV1}</span>
                      </td>
                      <td className="p-3.5 sm:p-4 text-[#141414] dark:text-[#FFFFFF]">
                        {v.v2} <span className="text-[10px] text-[#50585C] dark:text-[#7A8992] font-normal">{v.ipaV2}</span>
                      </td>
                      <td className="p-3.5 sm:p-4 text-[#141414] dark:text-[#FFFFFF]">
                        {v.v3} <span className="text-[10px] text-[#50585C] dark:text-[#7A8992] font-normal">{v.ipaV3}</span>
                      </td>
                      <td className="p-3.5 sm:p-4">
                        <span className="px-2 py-0.5 rounded bg-black/5 dark:bg-white/5 text-[10px] text-[#50585C] dark:text-[#BFD8E3]">
                          {v.pattern}
                        </span>
                      </td>
                      <td className="p-3.5 sm:p-4 text-[#50585C] dark:text-[#7A8992]">{v.meaningId}</td>
        ...
        {activeTab === 'nouns' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filteredNouns.map((n) => (
              <div
                key={n.id}
                className="p-5 sm:p-6 rounded-3xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 shadow-xs space-y-3"
              >
                <div className="flex items-center justify-between pb-2 border-b border-[#BFD8E3]/30 dark:border-white/5">
                  <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded-md bg-[#00638E]/15 text-[#00638E] dark:text-[#8CB9CC]">
                    {n.category}
                  </span>
                  <span className="text-xs font-mono text-[#50585C] dark:text-[#7A8992]">{n.meaningId}</span>
                </div>
                <h4 className="text-lg font-serif font-bold text-[#141414] dark:text-[#FFFFFF]">
                  {n.singularForm} {n.pluralForm ? `→ ${n.pluralForm}` : ''}
                </h4>
                <p className="text-xs text-[#50585C] dark:text-[#7A8992] leading-relaxed">
                  {n.ruleExplanation}
                </p>
                <div className="p-3 rounded-xl bg-[#EDF3F7] dark:bg-[#1C1C1C] text-xs font-mono text-[#141414] dark:text-[#FFFFFF]">
            ...
            {PUNCTUATION_GUIDE_DATA.map((p) => (
              ...
              >
                <div className="flex items-center justify-between pb-2 border-b border-[#BFD8E3]/30 dark:border-white/5">
                  ...
                  <span className="text-xs font-mono font-semibold text-[#50585C] dark:text-[#7A8992]">{p.markName}</span>
                </div>
                <p className="text-xs sm:text-sm text-[#141414] dark:text-[#FFFFFF] leading-relaxed">
                  {p.primaryRule}
                </p>
                <div className="p-3 rounded-xl bg-[#EDF3F7] dark:bg-[#1C1C1C] text-xs font-mono text-[#141414] dark:text-[#FFFFFF]">
                  Contoh Baku: "{p.correctExample}"
                </div>
                <p className="text-[11px] text-[#50585C] dark:text-[#7A8992]">
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {HAVE_HAS_HAD_MASTER_DATA.map((item) => (
                <div
                  key={item.id}
                  className="p-5 rounded-3xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 shadow-xs space-y-3"
                >
                  <span className="text-xs font-mono font-bold px-2 py-0.5 rounded bg-[#00638E]/15 text-[#00638E] dark:text-[#8CB9CC]">
                    {item.role}
                  </span>
        ...
        {activeTab === 'xray' && (
          <div className="space-y-6">
            {XRAY_SENTENCES_DATA.map((xray) => (
              <div
                key={xray.id}
                className="p-6 sm:p-8 rounded-3xl bg-[#FFFFFF] dark:bg-[#141414] border border-[#BFD8E3]/40 dark:border-white/5 shadow-xs space-y-5"
              >
                <div className="space-y-2">
                  <span className="text-xs font-mono font-bold px-2.5 py-0.5 rounded-md bg-[#00638E]/15 text-[#00638E] dark:text-[#8CB9CC]">
                    Bedah Struktur Sintaksis Kompleks
                  </span>
                  <h3 className="text-xl font-serif font-bold text-[#141414] dark:text-[#FFFFFF]">
                    {xray.title}
                  </h3>
                  <p className="text-sm font-mono text-[#141414] dark:text-[#FFFFFF] p-4 rounded-2xl bg-[#EDF3F7] dark:bg-[#1C1C1C] border border-[#BFD8E3]/30 dark:border-white/5 leading-relaxed">
                    "{xray.fullSentence}"
                  </p>
                  <p className="text-xs text-[#50585C] dark:text-[#7A8992] italic">
                    Terjemahan: {xray.translation}
                  </p>
                </div>

                <div className="space-y-3 pt-2">
                  <span className="text-xs font-mono uppercase tracking-wider font-bold text-[#50585C] dark:text-[#7A8992] block">
                  ...
                  <div className="space-y-2">
                    {xray.breakdown.map((b, bIdx) => (
                      ...
                      >
                        ...
                        <span className="text-[11px] opacity-80 shrink-0">{b.explanation}</span>
                      </div>
                    ))}
                  </div>
                </div>
